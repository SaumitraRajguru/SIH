...
from quiz.models import Question, Field
import random

class Command(BaseCommand):
    help = 'Seeds the database with initial questions and fields.'

    def handle(self, *args, **options):
        self.stdout.write("Seeding data...")

        # Step 1: Create Fields based on your original categories
        fields_data = [
            ("Interest", "Questions about personal interests."),
            ("Degree", "Questions about educational degrees."),
            ("Career", "Questions about specific career paths."),
        ]
        
        created_fields = {}
        for name, desc in fields_data:
            field, created = Field.objects.get_or_create(name=name, defaults={'description': desc})
            created_fields[name] = field
        self.stdout.write(self.style.SUCCESS("Fields seeded."))

        # Step 2: Use your exact list of 28 questions and link them to fields
        all_questions = [
            {
        ...
        for question_data in all_questions:
            ...
            field_instance = created_fields.get(field_name)
            if field_instance:
                Question.objects.get_or_create(text=question_data['text'], field=field_instance)
        
        self.stdout.write(self.style.SUCCESS(f'Successfully added {len(all_questions)} new questions!'))